...
def _grid_line_with_colons(colwidths, colaligns, **kwargs):
    ...
    if not colaligns:
        colaligns = [""] * len(colwidths)
    segments = "+".join(_grid_segment_with_colons(w, a) for a, w in zip(colaligns, colwidths))
    return f"+{segments}+"
...
def _asciidoc_row(is_header, *args, **kwargs):
    ...
    def make_header_line(is_header, colwidths, colaligns):
        ...
        asciidoc_column_specifiers = [f"{align}{width:d}" for width, align in asciidoc_alignments]
...
def _align_column(
    ...
    if is_multiline:
        ...
        if not enable_widechars and not has_invisible:
            ...
            visible_widths = [maxwidth - (w - ln) for w, ln in zip(s_widths, s_lens)]
            # wcswidth and _visible_width don't count invisible characters;
            # padfn doesn't need to apply another correction
    ...
    if hasattr(tabular_data, "keys") and hasattr(tabular_data, "values"):
        ...
        if headers == "keys" and not rows:
            ...
            if headers == "keys":
                headers = field_names
            rows = [
                ([getattr(row, f) for f in field_names] if not _is_separating_line(row) else row)
                for row in rows
            ]
...
def tabulate(
    ...
    if headers:
        ...
        minwidths = [
            max(minw, max(width_fn(cl) for cl in c)) for minw, c in zip(minwidths, t_cols)
        ]
        headers = [
...
def _append_multiline_row(
    ...
    cells_lines = [
        _align_cell_veritically(cl, nlines, w, rowalign) for cl, w in zip(cells_lines, colwidths)
    ]
